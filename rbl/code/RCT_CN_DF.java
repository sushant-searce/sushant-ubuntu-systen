...
public class RCT_CN_DF {

	public static void main(String[] args) {
		
		System.out.println("RCT => REF_REC_TYPE = 'CN'");
		...
		Dataset<Row> RCT_CN = MRCT.select(MRCT.col("REF_CODE"), MRCT.col("REF_DESC"))
				.filter(MRCT.col("REF_REC_TYPE").equalTo("CN"))
		...
		Dataset<Row> RCT_CN_DF  = RCT_CN.flatMap(new FlatMapFunction<Row, Row>() {
            ...
            public Iterator<Row> call(Row record) throws Exception {
                String RCT_CN_REF_CODE = record.getAs("RCT_CN_REF_CODE");
                String RCT_CN_REF_DESC = record.getAs("RCT_CN_REF_DESC");
