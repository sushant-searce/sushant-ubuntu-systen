...
class RedshiftUpsertOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        #         sql_statement += " and "
        sql_statement += "insert into " + self.dest_table + " with cte as (select id, customer_id, credit_limit_sys, customer_id_sys, remaining_credit_limit, balance_sys, ROW_NUMBER() over (partition by id order by __ts_ms desc) as rnum from " + self.src_table + " where __deleted='false')select id, customer_id, credit_limit_sys, customer_id_sys, remaining_credit_limit, balance_sys from cte where rnum=1;"
        sql_statement += "; "
