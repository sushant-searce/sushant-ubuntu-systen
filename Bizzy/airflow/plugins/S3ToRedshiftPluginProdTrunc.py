...
class S3ToRedshiftOperatorProdTrunc(BaseOperator):
...
#        self.cdc_prefix=kwargs['op_kwargs']['cdc_prefix']
    ...
    #    self.temp_prefix=kwargs['op_kwargs']['temp_prefix']
        ...
        super(S3ToRedshiftOperatorProdTrunc, self).__init__(*args, **kwargs)
    ...
    def execute(self, context):
       ...
       # load_statement = "truncate "+self.cdc_prefix+'.'+self.table+" ; ;"
        ...
        s3_path = self.s3_path+self.table+"/"+lastHourS3Path+"/"+self.table+".json"
        print(s3_path)
        ...
        try:
            ...
            if 'Contents' in responseTwo:
                ...
                load_statement = """
                    ...
                    json '{4}' timeformat 'YYYY-MM-DD HH:MI:SS' """.format(

                self.main_prefix+"."+self.table, self.s3_bucket, s3_path, self.iam_role, s3_copy_format)
                ...
                s3Client.delete_object(Bucket=self.s3_bucket,Key=s3_path)
...
class S3ToRedshiftOperatorPluginProdTrunc(AirflowPlugin):

    name = "redshift_load_plugin_production_trunc"

    operators = [S3ToRedshiftOperatorProdTrunc]
