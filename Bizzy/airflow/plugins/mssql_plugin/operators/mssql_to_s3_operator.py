...
class MsSQLToS3Operator(BaseOperator):
    ...
    def get_records_batch(self, hook, query_filter):
        ...
        count_sql_max = """
        SELECT count({0}) as c FROM {1} """.format(
            self.primary_key,
            self.mssql_table)
        ...
        with smart_open.smart_open(url, 'wb') as fout:
            ...
            for batch in range(0, count, self.batchsize):
                # query = \
                #     """
                #     SELECT  *
                #     FROM {table}
                ...
                query = """select * from {table} order by {primary_key} OFFSET {batch} ROWS FETCH FIRST {batchsize} ROWS ONLY ;""".format(table=self.mssql_table,primary_key=self.primary_key,batch=batch,batchsize=self.batchsize)
