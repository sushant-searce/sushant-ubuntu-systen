...
prefix_s3_path = "debezium-connector/prod-innovation-cluster/prod-innovation-cluster.public."
...
#dag = DAG('ThingsBoardAirflow', default_args=default_args, schedule_interval='@hourly')
dag = DAG('ThingsBoardAirflow', default_args=default_args, schedule_interval='@hourly')
...
def load(number, **kwargs):
    return S3ToRedshiftOperatorProd(
        ...
        op_kwargs={"s3_past_two_hour_path":s3_past_two_hour_path, "cdc_prefix":"thingsboard_prod_cdc","temp_prefix":"thingsboard_prod_temp","main_prefix":"thingsboard_prod", "prefix_s3_path":prefix_s3_path, "iam_role":"arn:aws:iam::593305529235:role/s3-redshift-role"},
        dag=dag)
...
def upsert(tablename,columnname,primary_key_column, **kwargs):
    return RedshiftUpsertOperatorProd(
        ...
        op_kwargs={"cdc_prefix":"thingsboard_prod_cdc","temp_prefix":"thingsboard_prod_temp","main_prefix":"thingsboard_prod"},
        dag = dag)

schema_details_query = """
select * from schema_details where db_name='thingsboard_prod' and table_name in ('admin_settings','attribute_kv','customer','device','device_credentials','entity_view','event','relation','rule_chain','ts_kv_latest','user_credentials','ts_kv')
"""
