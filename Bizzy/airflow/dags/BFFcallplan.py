...
prefix_s3_path = "debezium-connector/prod-crowdsys-apps-farmer-cluster/prod-crowdsys-apps-farmer-cluster.callplan_prod."
...
dag = DAG('BFFCallplanAirflow', default_args=default_args, schedule_interval='@hourly')
...
def load(number, **kwargs):
    return S3ToRedshiftOperatorProd(
        ...
        op_kwargs={"s3_past_two_hour_path":s3_past_two_hour_path, "cdc_prefix":"callplan_prod_cdc","temp_prefix":"callplan_prod_temp","main_prefix":"callplan_prod", "prefix_s3_path":prefix_s3_path, "iam_role":"arn:aws:iam::593305529235:role/s3-redshift-role"},
        dag=dag)
...
def upsert(tablename,columnname,primary_key_column, **kwargs):
    return RedshiftUpsertOperatorProd(
        ...
        op_kwargs={"cdc_prefix":"callplan_prod_cdc","temp_prefix":"callplan_prod_temp","main_prefix":"callplan_prod"},
        dag = dag)

schema_details_query = """
select * from schema_details where db_name='callplan_prod' and table_name in ('callplan','daily_route','non_sales_reason','non_visit','non_visit_reason','out_route_log','routine_schedule','routine_schedule_item','visit','visit_activity')
"""
