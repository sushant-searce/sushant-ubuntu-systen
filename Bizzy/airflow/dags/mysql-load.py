...
dag = DAG('mysql-load-data', default_args=default_args, schedule_interval=None)
...
def load(number,db_name, **kwargs):
    return S3ToRedshiftOperatorProduction(
        ...
        op_kwargs={"s3_past_two_hour_path":s3_past_two_hour_path, "main_prefix":db_name, "prefix_s3_path":prefix_s3_path, "iam_role":"arn:aws:iam::593305529235:role/s3-redshift-role"},
        dag=dag)


def upsert(tablename,db_name,**kwargs):
...
for record in myDatabaseHook.get_records(schema_details_query):
        ...
        Mysql_to_s3(record[2],conn) >> upsert(record[2],record[1]) >> load(record[2],record[1])
