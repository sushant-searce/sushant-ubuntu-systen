...
class SageMakerOperatorBuilder:
    ...
    def callableFunction(self,taskid,states,context,DYNMODB_TABLE_NAME,workflow_execution_id,**kwargs):
        ...
        if Type=='framework':
            ...
            script_location = os.path.split(inference_code)
            s3_location = script_location[0]
            name_bucket = s3_location.split('/')[2]
            name_key = '/'.join(s3_location.split('/')[3:])
            path = "/root/airflow/sagemaker/execution-{}-{}".format(context['executionId'],str(uuid.uuid4().hex[:4]))
            ...
            name_filee = inference_code.split('/')[-1]          
            file_list=[]
            ...
            for i in file_list:
                if i==name_filee:
                    inference_script_location=path+"/"+i
                else:
                    pass   
            print("inference file location:", inference_script_location)
