...
context = {'user': 'mohan.paralla', 'team': 'Proactive Customer Care Task', 'department': 'Cloud Solution Department', 'executionId': '719471359'}
def read_deployment_yaml():
    file_name= "public/airflow/definationyaml/719471359/definition.yaml"
...
def on_failure_callback(context):
    ...
    payload1 = '{"workflow_execution_id" : "719471359"}'
...
def update_stopped_status(**kwargs):
    ...
    payload = '{"workflow_execution_id" : "719471359"}'
...
def create_dynamic_module(task_type, task_id, resource_type, states):
    ...
    if task_type == "Task":
        if resource_type == 'arn:aws:states:::lambda:invoke':
            ...
            print(ecs_type)
            if ecs_type =='ecs':
                dynamicTask = ECSOperatorBuilder().getOperator(dag, task_id, states, 719471359)
            elif ecs_type=='ecs-ec2':
                dynamicTask = ECSEC2ALLOperatorBuilder().getOperator(dag, task_id, states, 719471359)
