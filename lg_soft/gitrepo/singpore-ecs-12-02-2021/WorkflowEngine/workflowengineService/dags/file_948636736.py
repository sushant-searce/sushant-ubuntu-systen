...
context = {'user': 'pshinde', 'team': 'Big Data CI Task', 'department': 'Big Data Department', 'executionId': '948636736'}
def read_deployment_yaml():
    file_name= "public/airflow/definationyaml/948636736/definition.yaml"
...
def on_failure_callback(context):
    ...
    payload1 = '{"workflow_execution_id" : "948636736"}'
...
def update_stopped_status(**kwargs):
    ...
    payload = '{"workflow_execution_id" : "948636736"}'
...
def create_dynamic_module(task_type, task_id, resource_type, states):
    ...
    if task_type == "Task":
        if resource_type == 'arn:aws:states:::lambda:invoke':
            ...
            print(ecs_type)
            if ecs_type =='ecs':
                dynamicTask = ECSOperatorBuilder().getOperator(dag, task_id, states, 948636736)
            elif ecs_type=='ecs-ec2':
                dynamicTask = ECSEC2ALLOperatorBuilder().getOperator(dag, task_id, states, 948636736)
