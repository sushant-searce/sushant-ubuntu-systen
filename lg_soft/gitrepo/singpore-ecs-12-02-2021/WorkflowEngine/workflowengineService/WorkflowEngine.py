...
def airflow_step(workflow_version_id, definition, context, deployment_yaml):
    ...
    if definition.get("id") is not None:
        ...
        return AirflowFactory().getStepImplementation(context, platformModuleId, step_config, config_string, workflow_version_id,module_id, deployment_yaml)
    ...
    for step_name, step_definition in modules.items():
        ...
        if type(step_definition) is not str and constants.ORCHESTRATION_ENGINE == 'Airflow':
            if 'Choice' not in step_name:
                step = airflow_step(workflow_version_id, step_definition, context, deployment_yaml)
                context['step_execution'][step_name] = step
        else:
            step = build_step(workflow_version_id, step_definition, context)
            context['step_execution'][step_name] = step
