...
def on_failure_callback(context):
    ...
    payload1 = '{"workflow_execution_id" : "searceLG"}'
...
def update_stopped_status(**kwargs):
    ...
    payload = '{"workflow_execution_id" : "searceLG"}'
    ...
    if task_type == "Task":
        if resource_type == 'arn:aws:states:::lambda:invoke':
            ...
            if ecs_type =='ecs':
                ...
                dynamicTask = ECSEC2OperatorBuilder().getOperator(dag, task_id, states, searceLG)
...
create_dynamic_branch(updated_definition, start_worflow, end_workflow)""".replace('searceLG', str(workflow_execution_id)).replace("WORKFLOW_EXECUTION_YAML_BUCKET", constants.SC_S3_BUCKET).replace("CONTEXT", str(tag_context))
   
    file_name = "dags/file_"+str(workflow_execution_id)+".py"
    open(file_name, "w").write(definition)
    print("please check the DAG ID : lg-soft-"+str(workflow_execution_id))
    os.system("scp -i searce-ap-southeast-1-kp.pem {} ubuntu@ec2-52-220-145-80.ap-southeast-1.compute.amazonaws.com:/root/airflow/dags".format(file_name))
