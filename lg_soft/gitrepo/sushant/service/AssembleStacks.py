...
def build_ecs_instance(definition, context, t):
    ...
    ec2_name=definition.get("name")
    ec2_instance_name=ec2_name.replace(" ", "")+str(res)
    ...
    image_id = json.loads(json_string)['image_id']
    ...
    ec2_instance = t.add_resource(ec2.Instance(
        ...
        Tags=[{'Key':'Name','Value':'ECS-EC2 Instance'},
              {'Key':'user','Value':context['tags']['user']},
...
def build_emr_stack(definition,context,t):
    ...
    workflow_version_id = str(context['workflow_version_id'])
    executionId = str(context['tags']['executionId'])
    logUri = "s3://"+constants.SC_S3_BUCKET+"/public/Workflow/ExecutionFiles/"+workflow_version_id+"/"+executionId+"/elasticmapreduce/"
...
def build_cft_definition(config_yaml, context):
    ...
    for deployment in deployments:
        ...
        if deployment_type == "function":
            print("module_id",deployment.get('moduleid'))
            zip_location = zip_s3_files(deployment.get('moduleid'))
            #layer_zip_location = layer_zip(deployment.get('moduleid'))
            #outputs.append(build_lambda_layer_stack(deployment, layer_zip_location, t))
