...
org_mgmt_table = dynamodb.Table(constants.ORG_MGMT_TABLE)
...
def check_and_update_execution_status(workflow_version_id, context):
    ...
    if len(items) < 1:
        ...
        response = table.get_item(Key=key)['Item']
        workflow_execution_version_id = str(round(time.time()*1000))[2:-2]
...
def update_failed_workflow(context):
    if not context.get('tags').get('executionId'):
        return True
    key = {
        "id": int(context['tags']['executionId']),
        "type": "workflowExecution"
    }
    response = table.get_item(Key=key).get('Item')
    if response:
        if response.get('execution_status') == 'INITIALIZING':
...
def get_container_uri(module_id):
    ...
    try:
        response = table.get_item(Key=key1)['Item']
    except:
        response =  table.get_item(Key=key2)['Item']
...
def config_sanity_Check(workflow_version_id):
    workflow_yaml = fetch_config_from_db(workflow_version_id, constants.WORKFLOW_CONFIG_VERSION)
    ...
    for k, v in steps.items():
        if(k != "start"):
            if(v.get('end') != "True"):
                if(v.get('type') == 'Task'):
                    if 'Choice' in v.get('next'):
                        if 'Customcode' in steps[v.get('next')]['conditions'][0]['Variable']:
                            statuscount = 0
                            truecount = 0
                            print(v['next'])
                            moduleid = workflow_yaml['modules'][k]['id']
                            print(moduleid)
                            outputs = fetch_module_config_from_db(moduleid,workflow_version_id)['config']['output']['dest']
                            for k in outputs:
                                if k.get('status'):
                            ...
                            if(statuscount==0 or truecount!=1):
                                return False
    return True

def get_team_details(team, context):
    teamData = org_mgmt_table.query(
            KeyConditionExpression=Key('type').eq('team'),
            FilterExpression=Attr('itemName').eq(team),
            ProjectionExpression='id, parent'
        )['Items']
    if teamData:
        context['tags']['team'] = str(teamData[0]['id'])
        context['tags']['department'] = str(teamData[0]['parent'])
    else:
        return False
    return True

def get_cluster_details(teamId):
    key = {
        "id": int(teamId),
        "type": "team"
    }
    response = org_mgmt_table.get_item(Key=key, ProjectionExpression='teamCluster, instanceType, maxInstanceCount')['Item']
    return response
